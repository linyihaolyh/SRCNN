...
def resizeSave(scale_factor,img_input_path,img_out):
    img = cv2.imread(img_input_path)
    out=cv2.resize(img, (img.shape[1]//scale_factor, img.shape[0]//scale_factor), interpolation=cv2.INTER_CUBIC)
    cv2.imwrite(img_out, out)
    
def interpolationSave(scale_factor,img_input_path,img_out,interpolation):
    img = cv2.imread(img_input_path)
    out=cv2.resize(img, (img.shape[1]//scale_factor, img.shape[0]//scale_factor), interpolation=interpolation)
    out=cv2.resize(out, (img.shape[1], img.shape[0]), interpolation=interpolation)
    cv2.imwrite(img_out, out)
